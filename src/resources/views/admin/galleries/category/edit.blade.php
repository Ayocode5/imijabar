...
@section('admin_content')
    ...
    <form action="{{ route('admin.galleries.category.update',$category->id) }}" method="post">
        @csrf
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 mt-2 font-weight-bold text-primary">Category</h6>
                <div class="float-right d-inline">
                    <a href="{{ route('admin.galleries.category.index') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> View All</a>
                </div>
            </div>
            <div class="card-body">
                <div class="form-group">
                    <label for="">Name *</label>
                    <input required type="text" name="name" class="form-control" value="{{ $category->name }}" autofocus>
                </div>
            </div>
            <div class="card-header py-3">
